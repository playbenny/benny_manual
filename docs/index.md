...
other key ideas behind benny are that it provides a minimal, low distraction interface that's easy to glance at (eg while performing) and that it tries to avoid imposing any heirarchical decisions on the user (eg it has no global timeline, no pre-defined mixer structure, etc)
